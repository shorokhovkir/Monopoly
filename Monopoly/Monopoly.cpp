...
void Monopoly::movePlayer(int playerID, int currRollCount, sf::RenderWindow& window, int dealChoice = -1, bool toJail = false) {
	...
	if (!board.getPlayerByID(playerID)->getIsBankrupt()) {
		...
		if (!player->getIsInJail()) {
			...
			playerPosition[playerID] %= 40; // ¬ÂÌÛÚ¸Òˇ, ÂÒÎË ·Û‰ÛÚ ‚˚Ô‡‰‡Ú¸ ‰Û·ÎË ÏÌÓ„Ó ‡Á ÔÓ‰ˇ‰
			...
			if (strcmp(board.getCells()[playerPosition[playerID]]->getSpaceType(), "PRIVATE") == 0) {
				...
				if (player->getPlayerID() != property->getOwnerID()) {
					...
					sf::RenderWindow buyOrRentPrompt(sf::VideoMode(265, 166), " ÛÔËÚ¸ ËÎË ‡ÂÌ‰Ó‚‡Ú¸", sf::Style::Titlebar);
						...
						while (buyOrRentPrompt.pollEvent(evt)) {
							...
							if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
								...
								if (closeBtnBounds.contains(mousePos)) {
									...
									sf::RenderWindow bid(sf::VideoMode(265, 166), "“Œ–√»", sf::Style::Titlebar);
									...
									while (bid.isOpen()) { // ¬ÂÌÛÚ¸Òˇ
										...
										currTurnDisplay.setString("’Œƒ: " + to_string(bidTurn + 1));
					...
					if (player->getCash() >= property->getPurchasePrice() && dealChoice == 0) {
						...
						if (!barter) {
							...
							sf::RenderWindow prompt(sf::VideoMode(280, 90), "Õ≈ƒŒ—“¿“Œ◊ÕŒ —–≈ƒ—“¬!", sf::Style::Titlebar);

							sf::Text nec;
							nec.setString("Õ≈ƒŒ—“¿“Œ◊ÕŒ —–≈ƒ—“¬\nœŒœ–Œ¡”…“≈ ¿–≈ÕƒŒ¬¿“‹\n.");
			...
			if (strcmp(board.getCells()[playerPosition[playerID]]->getSpaceType(), "COMMERCIAL") == 0) {
				...
				if (player->getPlayerID() != property->getOwnerID()) {
					...
					sf::RenderWindow buyOrRentPrompt(sf::VideoMode(265, 166), "œÓÍÛÔÍ‡ ËÎË ‡ÂÌ‰‡", sf::Style::Titlebar);
			...
			if (strcmp(board.getCells()[playerPosition[playerID]]->getSpaceType(), "COMMUNITY") == 0) {
				...
				sf::RenderWindow card(sf::VideoMode(500, 250), "—”Õƒ”  ”ƒ¿◊»", sf::Style::Titlebar);
			...
			if (strcmp(board.getCells()[playerPosition[playerID]]->getSpaceType(), "CHANCE") == 0) {
				...
				sf::RenderWindow card(sf::VideoMode(500, 250), " ¿–“¿ ÿ¿Õ—¿", sf::Style::Titlebar);
			...
			if (strcmp(board.getCells()[playerPosition[playerID]]->getSpaceType(), "LANDTAX") == 0) {
				...
				sf::RenderWindow taxPrompt(sf::VideoMode(300, 160), "«ÂÏÂÎ¸Ì˚È Ì‡ÎÓ„", sf::Style::Titlebar);


				sf::Text cardTitle;
				cardTitle.setString("«ÂÏÂÎ¸Ì˚È Ì‡ÎÓ„.");
				cardTitle.setCharacterSize(25);
				cardTitle.setFont(stdFont);
				cardTitle.setPosition(15.0f, 15.0f);
				cardTitle.setFillColor(sf::Color::White);
				...
				sf::Text cardText;
				cardText.setFont(cardFont);
				cardText.setString("20% ÁÂÏÂÎ¸ÌÓ„Ó Ì‡ÎÓ„‡ ·˚ÎÓ ‚˚˜ÚÂÌÓ!");
				cardText.setPosition(15.0f, 100.0f);
			...
			if (strcmp(board.getCells()[playerPosition[playerID]]->getSpaceType(), "GOTOJAIL") == 0) {
				...
				sf::Text cardText;
				cardText.setFont(cardFont);
				cardText.setString("¬˚ ÔÓÔ‡ÎË ‚ Ú˛¸ÏÛ.\n¬˚·ÂË ‰ÂÈÚ‚ËÂ, ÍÓÚÓÓÂ\n‚˚ ıÓÚËÚÂ ÒÓ‚Â¯ËÚ¸.");
				...
				btnText[0].setString("\t »—œ€“¿“‹\n”ƒ¿◊”");
				btnText[1].setString("»—œŒÀ‹«Œ¬¿“‹  ¿–“”");
				btnText[2].setString("¬Õ≈—“» «¿ÀŒ√");
				btnText[3].setString(" ”œ»“‹  ¿–“”");
								...
								if (btnBound[i].contains(mousePos)) {
									...
									if (i == 0) {
										...
										if (player->getHasJailRescueCard()) {
											...
											err.setString("” ‚‡Ò ÌÂÚ Í‡Ú˚ ÒÔ‡ÒÂÌËˇ ËÁ Ú˛¸Ï˚!");
										}
										...
										if (player->getCash() >= 400) {
											...
											for (int i = 0; i < board.getPlayerCount() && jailCardExists == -1; i++) {
												...
												cout << "»„ÓÍ " << i << " -> " << allPlayers[i]->getHasJailRescueCard() << endl;
											...
											if (jailCardExists != -1) {
												...
												sf::RenderWindow bin(sf::VideoMode(250, 120), "ƒ¿ »À» Õ≈“?", sf::Style::Titlebar);
												...
												for (int i = 0; i < 2; i++) {
													...
													ynText[i].setString((i == 0) ? "ƒ¿" : "Õ≈“");

												}

												sf::Text note;
												note.setString("¬˚ ıÓÚËÚÂ ÔÓ‰‡Ú¸\nÍ‡ÚÛ?");
													...
													while (bin.pollEvent(bEvt)) {
														...
														if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
															...
															if (ynBounds[0].contains(mousePos)) {
																...
																err.setString("»„ÓÍ ÓÚÍ‡Á‡ÎÒˇ ÓÚ Ò‰ÂÎÍË.");
												...
												err.setString("ÕËÍÚÓ ÌÂ ‚Î‡‰ÂÂÚ \"Í‡ÚÓÈ ÒÔ‡ÒÂÌËˇ\" Ì‡ ‰‡ÌÌ˚È ÏÓÏÂÌÚ.");
											...
											err.setString("ÕÂ‰ÓÒÚ‡ÚÓ˜ÌÓ ÒÂ‰ÒÚ‚");
...
void Monopoly::mortgageProperty(int cellNum) {
	...
	if (p->getOwnerID() != -1 && !p->getMortgaged()) {
		...
		if (board.getPlayerByID(p->getOwnerID())->getCash() >= ((propertyValue / 2) + (0.2 * propertyValue))) {
			...
			sf::Text nec;
			nec.setString("Õ≈ƒŒ—“¿“Œ◊ÕŒ —–≈ƒ—“¬!\n¬‡Ï ÌÂÓ·ıÓ‰ËÏÓ Ì‡ 20% ·ÓÎ¸¯Â\nÓÚ ËÁÌ‡˜‡Î¸ÌÓÈ ÒÚÓËÏÓÒÚË.");
			nec.setPosition(5.0f, 5.0f);
			nec.setFont(stdFont);
			nec.setCharacterSize(15);
			nec.setFillColor(sf::Color::Black);
