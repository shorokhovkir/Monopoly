...
public:
	...
	Map(const char* key, int value, int a, int b, int c = 0, int d = 0) { // Âåðíóòüñÿ
			...
			if (i == 0 && j == 0) {
				...
				if (owner->getCash() >= 100) {
					...
					if (p->getHouseCount() == minHouses) {
						...
						sf::RenderWindow prompt(sf::VideoMode(280, 90), "ÍÅÄÎÑÒÀÒÎ×ÍÎ ÑÐÅÄÑÒÂ!", sf::Style::Titlebar);

						sf::Font stdFont;
						stdFont.loadFromFile("fonts/Montserrat-Black.ttf");

						sf::Text nec;
						nec.setString("ÍÅÂÎÇÌÎÆÍÎ ÏÎÑÒÐÎÈÒÜ\nÄÎËÆÍÛ ÁÛÒÜ\nÂÛÊÓÏËÅÍÛ ÂÑÅ ÄÎÌÀ."); //Âåðíóòüñÿ!
						...
						closeBtnTexture.loadFromFile("elements/upgrade_prompt_close.png");
...
void Board::putsPlayerOnSpace(int index, int playerID, sf::RenderWindow& window, int dealChoice) { // Âåðíóòüñÿ
